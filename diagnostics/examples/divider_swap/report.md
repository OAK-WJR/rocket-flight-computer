...
| DC_3V3A | PASS | 3V3A measured interval=[3.2880,3.2920] V. 3.3 V ±5% is only a DC screening window, not proof that every chip or transient is acceptable. |
| POWER_PATH | INFO | None of the three severe power-drop criteria triggered. This is not a transient / thermal stability pass. |
| DIV_VLOGIC | FAIL | Measured interval [4.17800,4.18200] V; the 1% resistor + meter input loading model expects [0.41009,0.42564] V. Overlap only means consistency with this model; if they do not overlap, check wiring / meter / fitted resistor values first, then the divider and ADC side. |
